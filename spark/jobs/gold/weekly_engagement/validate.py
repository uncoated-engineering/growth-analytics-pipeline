"""Data quality validation for gold weekly_engagement."""
...
def validate_input(spark: SparkSession, bronze_path: str = "data/bronze") -> int:
    """Validate the bronze feature_usage_events table before aggregation."""
    return validate_delta_table(
        spark,
        path=f"{bronze_path}/feature_usage_events",
        expected_schema=FEATURE_USAGE_EVENTS_OUTPUT_SCHEMA,
        table_name="gold.weekly_engagement (input: bronze.feature_usage_events)",
    )


def validate_output(spark: SparkSession, gold_path: str = "data/gold") -> int:
    """Validate the gold weekly_engagement Delta table after aggregation."""
    return validate_delta_table(
        spark,
        path=f"{gold_path}/gold_weekly_engagement",
        expected_schema=WEEKLY_ENGAGEMENT_SCHEMA,
        table_name="gold.weekly_engagement (output)",
    )
...
if __name__ == "__main__":
    ...
    builder = (
        SparkSession.builder.appName("Validate gold.weekly_engagement")
